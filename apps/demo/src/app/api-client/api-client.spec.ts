...
fdescribe('ApiClient', () => {
  ...
  it('getMazeById should make GET requesut to fetch a maze by Id.', () => {
    apiClient.getMazeById(MOCK_MAZES[0].id).subscribe((response) => {
      expect(response).toEqual(MOCK_MAZES[0]);
    });

    const req = httpMock.expectOne(`${baseUrl}/Maze/${MOCK_MAZES[0].id}`);
    expect(req.request.method).toBe('GET');
    req.flush(MOCK_MAZES[0]);
  });
